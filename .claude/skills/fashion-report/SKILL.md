...
まず `config/profile.json` を Read で読み込む。
プロフィールに性別・年代・体型・好みのスタイルなどが設定されている場合は、
ヒアリングをスキップしてその情報を使う。
未設定の項目のみ Step 1 で質問する。全項目が揃っていれば Step 1 を完全にスキップする。
...
以下の6項目をユーザーに質問する。
一度にまとめて聞いてOK。ユーザーが既に情報を提供している場合やプロフィールに設定済みの場合はスキップする。
